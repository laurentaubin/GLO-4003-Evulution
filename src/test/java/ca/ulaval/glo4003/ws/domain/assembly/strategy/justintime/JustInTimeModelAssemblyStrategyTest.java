...
class JustInTimeModelAssemblyStrategyTest {
  ...
  private static final Model FIRST_MODEL_WITH_PRODUCTION_TIME_OF_TWO_WEEKS =
          new ModelBuilder()
                  .withName(FIRST_MODEL_TYPE)
                  .withProductionTime(PRODUCTION_TIME_OF_TWO_WEEKS)
                  .build();
  private static final ModelOrder FIRST_MODEL_ORDER_WITH_PRODUCTION_TIME_OF_TWO_WEEKS =
          new ModelOrderBuilder().withOrderId(FIRST_ORDER_ID).withModelName(FIRST_MODEL_TYPE).withProductionTime(PRODUCTION_TIME_OF_TWO_WEEKS).build();
  ...
  givenOneOrderInQueue_whenComputeRemainingTimeToProduce_thenReturnOrderRemainingTimeToProduce() {
    ...
    given(modelOrderFactory.create(FIRST_MODEL.getName(), FIRST_MODEL.getProductionTime())).willReturn(FIRST_MODEL_ORDER);
    List<Model> modelsToAssemble = List.of(FIRST_MODEL);
    JustInTimeModelAssemblyStrategy justInTimeModelAssemblyStrategy = new JustInTimeModelAssemblyStrategy(
    ...
    // then
    assertThat(productionTime).isEqualTo(order.getModel().getProductionTime());
  }

  @Test
  public void givenMultipleOrdersRequestingTheSameModelTypeAndMultipleModelsToAssemble_whenComputeRemainingTimeToProduce_thenReturnCorrectRemainingTimeToProduce() {
    // given
    Order order = createAnOrderWithModelType(FIRST_MODEL_TYPE);
    Order anotherOrder = new OrderBuilder().withOrderId(SECOND_ORDER_ID).withModel(FIRST_MODEL).build();

    given(modelOrderFactory.create(FIRST_MODEL.getName(), FIRST_MODEL.getProductionTime())).willReturn(FIRST_MODEL_ORDER);
    given(modelOrderFactory.create(SECOND_MODEL.getName(), SECOND_MODEL.getProductionTime())).willReturn(SECOND_MODEL_ORDER);
    ...
    // then
    ProductionTime expectedProductionTime = new ProductionTime(FIRST_MODEL.getProductionTime().inWeeks()).add(SECOND_MODEL.getProductionTime()).add(FIRST_MODEL.getProductionTime());
    assertThat(productionTime).isEqualTo(expectedProductionTime);
  }

    @Test
    public void givenCurrentOrderIsInAssembly_whenComputeRemainingTimeToProduce_thenReturnRemainingTimeRemainingTimeOfOrderInAssembly() {
      // given
      given(modelOrderFactory.create(FIRST_MODEL.getName(), FIRST_MODEL.getProductionTime())).willReturn(FIRST_MODEL_ORDER);
      List<Model> modelsToAssemble = List.of(FIRST_MODEL);
      JustInTimeModelAssemblyStrategy justInTimeModelAssemblyStrategy = new JustInTimeModelAssemblyStrategy(
  ...
  givenModelInInventoryAndAnOrderForThisModel_whenAddOrder_thenOnAdvanceAddModelOrderToModelAssemblyLine() {
    // given
    Order anOrder = createAnOrderWithModelType(FIRST_MODEL_TYPE);
    ModelOrder modelOrder = new ModelOrderFactory().create(FIRST_MODEL_TYPE, PRODUCTION_TIME_OF_ONE_WEEK);

    when(modelInventory.isInStock(FIRST_MODEL_TYPE)).thenReturn(true);
    when(modelOrderFactory.create(anOrder.getModel().getName(), PRODUCTION_TIME_OF_ONE_WEEK)).thenReturn(modelOrder);
    ...
    // then
    justInTimeModelAssemblyStrategy.advance();
    verify(modelInventory).removeOne(anOrder.getModel().getName());

    verify(assemblyLineAdapter).addOrder(modelOrder);
  }
  ...
  public void givenOrders_whenGetActiveOrders_thenReturnOrders() {
    // given
    Order order = createAnOrderWithModelType(FIRST_MODEL_TYPE);
    JustInTimeModelAssemblyStrategy justInTimeModelAssemblyStrategy = createJustInTimeModelAssemblyStrategy();
    justInTimeModelAssemblyStrategy.addOrder(order);
  ...
  private JustInTimeModelAssemblyStrategy createJustInTimeModelAssemblyStrategy() {
    ...
    given(modelOrderFactory.create(THIRD_MODEL_TYPE, PRODUCTION_TIME_OF_ONE_WEEK)).willReturn(THIRD_MODEL_ORDER);
